...
> 最終更新: 2026-03-27（`telos init` 対話型ウィザード化・preflight チェック追加）
...
├── projects/                   # プロジェクトごとのデータディレクトリ
│   └── main/                   # デフォルトプロジェクト（TELOS_HOME未設定時）
...
[User]
  ...
  │     ├─ 2. initial_intent 入力 → config.yaml に書き込み + モデルをデフォルト設定
  ...
  └─ telos start --loops N --model M [--name session-name]
   ...
   CLI (cli.py)
        │
        ├─ _preflight_check(): config.yaml のモデルからプロバイダー名を動的導出し
...
`telos init` はウィザード完了時に以下のフィールドを `config.yaml` に書き込む。

```yaml
initial_intent: "<ユーザー入力値>"
llm:
...
書き込みは `Settings.save()` (config.py:123) が担当し、`yaml.dump(self.model_dump())` で全フィールドを保存する。保存後は `reload_settings()` でキャッシュをバストする。
...
> **注意**: `max_steps`（15）, `consecutive_error_limit`（3）, `max_output_truncation`（8000）は `config.yaml` に記述が不要な場合が多い（Pydanticデフォルト値が使われる）。必要な場合のみ追記可能。
...
| `TELOS_HOME` | データディレクトリパス（デフォルト: `projects/main/`）|
...
| コマンド | 機能 |
|---|---|
| `telos init [--force] [--non-interactive]` | 対話型セットアップウィザード（APIキー検証・config.yaml 書き込み・Docker 起動・埋め込みモデル DL）。`--force` で確認なし再初期化、`--non-interactive` で全プロンプトスキップ（CI用） |
...
各プロジェクトは `projects/<name>/` 以下に独立した DB・ワークスペース・ログを持つ。`telos project switch` は `TELOS_HOME=projects/<name>` を `.env.local` に書き込み、次回起動から有効になる。デフォルトプロジェクトは `projects/main/`。

```
projects/
├── main/         ← デフォルト（TELOS_HOME未設定時）
│   ├── telos.db
│   ├── JOURNAL.md
│   ├── workspace/
│   └── agent.log
