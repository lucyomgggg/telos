...
> 最終更新: 2026-03-22（Session管理・CLIプロジェクト管理機能反映）
...
`Orchestrator.__init__` は `SessionRecord` を SQLite に作成し、`session_id` を全ループに付与する。`Orchestrator.shutdown()` はセッション統計（completed_loops, avg_score, total_cost_usd）を確定させ、永続ワークスペースを削除する。
...
| `telos start [--loops N] [--model M] [--name NAME] [--verbose]` | 自律ループ起動（セッション作成） |
| `telos stop` | 実行中エージェントの停止 |
| `telos status [--limit N] [--loops]` | セッション履歴表示（`--loops` でループ単位） |
| `telos logs [-n LINES] [-f]` | ログ表示（ライブフォロー対応） |
| `telos show [LOOP_ID] [--explain]` | 特定ループの詳細表示 |
| `telos report [-o FILE] [--top N]` | 全ループの包括的Markdownレポート生成 |
| `telos export [SESSION_ID] [--format json\|csv] [-o FILE]` | セッションデータをエクスポート |
| `telos reset [--yes]` | アクティブプロジェクトのDB・ワークスペース・ログを削除 |
| `telos dashboard` | インタラクティブTUIダッシュボード起動 |
| `telos project current` | アクティブプロジェクトを表示 |
| `telos project list` | 全プロジェクト一覧を表示 |
...
全ループのデータを使って包括的なMarkdownレポートを生成する。`--limit` オプションは廃止され、常に全履歴を対象とする。

| セクション | 内容 | データソース |
|---|---|---|
| Summary | 総ループ数・平均スコア・高スコア率・失敗数・総コスト | `get_dashboard_summary()` |
| Score Progression | 全ループのスコア時系列テーブル | `get_score_progression()` |
| Rubric Axis Averages | 評価軸別平均スコア（バー表示付き） | `get_score_breakdown_averages()` |
| Top Performers | 上位N件のループ詳細（スコア内訳・Criticのreasoning） | `get_score_progression()` + `get_loop()` |
| Learning Moments | 失敗→改善ペア（スコア差・レッスン・改善reasoning） | `get_failure_improvement_pairs()` |
| All Goals | 全目標一覧テーブル（スコア・ステータス・日時） | `get_goal_diversity()` |
| Cost Analysis | モデル別コスト・トークン統計 | `get_model_cost_stats()` |
| Workspace Artifacts | 永続ワークスペース内の全ファイル一覧（サイズ・更新日時） | ファイルシステム |

オプション:
- `-o FILE` / `--output FILE`: 出力先（デフォルト: `REPORT.md`）
- `--top N`: Top Performers に表示するループ数（デフォルト: 5）
