...
> 最終更新: 2026-03-27（GoalGeneratorへのworkspace_state注入: ループ間の行動一貫性を実装）
...
[User]
  ...
  └─ telos start --loops N --model M [--name session-name]
   ...
   AgentLoop.run_iteration(intent)
        ...
        │      ├─ SandboxManager.list_files()でworkspaceをスキャン → workspace_state取得
        │      │      （persistent/内の {loop_id[:8]}/{filename} を列挙）
        │      ├─ LLMにGoalSchema形式（title/success_criteria/output_path）で生成
        │      │      workspace_stateが空でない場合: CURRENT WORKSPACEセクションを注入
        │      │      （既存ファイルを無視した独立スクリプトの新規作成を禁止する制約付き）
...
`Orchestrator.__init__` は `SessionRecord` を SQLite に作成し、`session_id` を全ループに付与する。また `JournalWriter` を初期化してセッションヘッダーを `JOURNAL.md` に追記する。`Orchestrator.shutdown()` はセッション統計（completed_loops, avg_score, total_cost_usd）を確定させ、Session Summary を JOURNAL.md に追記する。永続ワークスペースは削除せず次セッションへ引き継ぐ（明示的に削除したい場合は `telos reset`）。
...
人間の具体的な指示に従うのではなく、`templates/goal_generation_system.txt` に定義された本能に基づいて自律的に目標を決定する。
...
5. **CONTINUITY PRINCIPLE**: `CURRENT WORKSPACE` が提供された場合、既存の成果物を起点とした目標を生成する。孤立したスクリプトの乱立を防ぎ、蓄積によるシステム成長を促す。Loop 1（workspaceが空）ではこの制約は適用されない。
...
workspace_state: List[Dict] # persistent/内の既存ファイル一覧（空=Loop 1）
...
**workspace_stateの注入タイミング:** `Orchestrator.run_iteration()` がQdrant検索直後に `SandboxManager.list_files()` を呼び出し、生成されたリストを `GoalGenerator.generate()` に渡す。`workspace_state` が空でない場合のみ `CURRENT WORKSPACE` セクションがプロンプトに追加される。
...
`telos init` はウィザード完了時に `initial_intent` を `telos.yaml` に書き込む。モデル名はユーザーが直接 `telos.yaml` を編集するか、環境変数で変更する。

`Settings.save()` は常に `telos.yaml` に書き込む（`config.yaml` は書き換えない）。
...
> **注意**: `projects/` ディレクトリが存在して `TELOS_HOME` が未設定の場合、`telos start` 実行時に警告が表示される。`.env.local` に `TELOS_HOME=projects/<name>` を明示的に設定すること。
