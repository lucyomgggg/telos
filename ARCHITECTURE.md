...
> 最終更新: 2026-03-27（設定システムの2層化: グローバル設定 + プロジェクト設定）
...
|---|---|
| 評価軸・重み | `rubric.json` |
| インフラ設定（Qdrant URL、Docker、コスト上限） | `~/.config/telos/config.yaml`（グローバル） |
| プロジェクト設定（モデル名、インテント、メモリ設定） | `telos.yaml`（プロジェクト固有・git管理対象） |
...
│   ├── db_models.py            # SQLAlchemy ORM（SessionRecord, LoopRecord, AuditLog）
...
[User]
  │
  ├─ telos init --global [--force]
  │     └─ ~/.config/telos/config.yaml を生成（インフラ設定のみ）
  │
  ├─ telos init [--force] [--non-interactive]
  │     │
  │     ▼
  │  プロジェクトセットアップウィザード (cli.py)
  ...
  └─ telos start --loops N --model M [--name session-name]
   ...
   CLI (cli.py)
        ...
        │
        ▼
   AgentLoop.__init__  ← SessionRecord を SQLite に作成（status="running"）
        │
        ▼  （loops 回繰り返す）
   AgentLoop.run_iteration(intent)
        ...
        │      ├─ SQLiteから品質加重履歴取得（get_quality_history）
        │      │      ├─ 高スコア（≥0.7）上位10件
        │      │      ├─ 直近の失敗（<0.3）5件
        │      │      └─ 直近5件（時系列的連続性）
        │      ├─ Qdrantクエリ: 直近の失敗が3連続ならinitial_intentで検索
        │      │      （ドメイン引力からの脱出）それ以外は直近ゴールタイトルで検索
        ...
        │      └─ output_path を loop_id[:8]/ でプレフィックス（ループ間ファイル衝突防止）
        │
        ...
        └─ 9. SQLiteにloop最終状態で保存
              （artifacts embed のみ: score > failure_threshold）

        └─ 9. JournalWriter でループ結果を JOURNAL.md に即時追記

  ループ完了後（AgentLoop.shutdown）
        ├─ JournalWriter で Session Summary を JOURNAL.md に追記
        ├─ SessionRecord を status="completed" に更新
        │      （completed_loops, avg_score, total_cost_usd を集計）
        └─ persistent workspace を shutil.rmtree でクリーンアップ
```
...
`Orchestrator.__init__` は `SessionRecord` を SQLite に作成し、`session_id` を全ループに付与する。また `JournalWriter` を初期化してセッションヘッダーを `JOURNAL.md` に追記する。`Orchestrator.shutdown()` はセッション統計（completed_loops, avg_score, total_cost_usd）を確定させ、Session Summary を JOURNAL.md に追記したあと、永続ワークスペースを削除する。
...
#### 品質加重履歴 (`get_quality_history`)

`GoalGenerator` に渡す履歴は単純な「直近N件」ではなく、品質加重のミックスを使用する：

| ロール | 条件 | 上限 |
|---|---|---|
| 成功パターン | score ≥ 0.7 | 10件 |
| 最近の失敗 | score < 0.3 | 5件 |
| 時系列的連続性 | 直近問わず | 5件 |

重複は除去されてマージされる。これによりLLMに成功例・失敗例・最近の流れをバランスよく提示できる。
...
**ドメイン脱出ヒューリスティック:** 直近3ループが全て `failure_threshold` 未満だった場合は `initial_intent` をQdrantクエリとして使用する（ドメイン引力から強制脱出）。
...
2. `telos.yaml` の `critic.rubric_path`
...
| critic_model | str | 使用したCriticモデル名 |
...
各プロジェクトの `JOURNAL.md` にセッション・ループの結果を自動追記する。ユーザーはこのファイルを直接開いてAIが何をしたか確認できる。
...
| `run_iteration()` 完了直後 | ループブロック（Goal/Result/Reasoning）を即時追記 |
| `Orchestrator.shutdown()` | Session Summary 行を追記 |
...
**Result:** abc12345/explorer.py
**Reasoning:** 成果物が要件を満たしており、実行を確認できた。

### Loop 2 ❌ 0.21
**Goal:** 外部APIへの接続を試みる
**Result:** ネットワーク制限によりタイムアウト。
**Reasoning:** サンドボックスのネットワーク制限で失敗、成果物なし。

---
**Session Summary:** 2 loops | avg score: 0.51 | cost: $0.031
---
```

スコアアイコン: `✅` (score ≥ 0.5) / `❌` (score < 0.5)

Result フィールド: `output_path` が設定されていればそれを使用、なければ `reasoning` の冒頭1〜2文（200文字上限）を使用。
...
設定は **グローバル設定** と **プロジェクト設定** の2層に分離されている。

| 層 | ファイル | 責務 | git管理 |
|---|---|---|---|
| グローバル | `~/.config/telos/config.yaml` | インフラ・マシン固有（Qdrant URL、Docker、コスト上限、ログレベル） | No |
| プロジェクト | `telos.yaml`（CWDから上方向に探索） | プロジェクト固有（モデル、インテント、メモリ設定、ルーブリックパス） | Yes |
...
環境変数
    ↓
telos.yaml（CWDから上方向に探索、見つからなければスキップ）
    ↓
~/.config/telos/config.yaml（グローバル）
    ↓
Pydanticデフォルト値
```

グローバルとプロジェクト設定は **deep merge** で合成される。同じキーがある場合はプロジェクト設定が優先。設定はモジュールレベルのキャッシュ（`_settings_cache`）で保持。`reload_settings()` でキャッシュを破棄して再読み込み可能。

#### `_find_project_config()`

CWDから親ディレクトリを順に遡って `telos.yaml` を探す。ルートまで見つからなければ `None` を返す（エラーにならない）。後方互換: `config.yaml` が見つかった場合は `DeprecationWarning` を出したうえで使用する。

#### `telos init` による自動書き込み

`telos init` はウィザード完了時に `initial_intent` のみを `telos.yaml` に書き込む。モデル名はPydanticデフォルトをそのまま使用（ウィザードで上書きしない）。モデル変更はユーザーが `telos.yaml` を直接編集するか、環境変数で行う。

書き込みは `Settings.save()` が担当。保存先は常にプロジェクト設定（`telos.yaml`）であり、グローバル設定には書き込まない。

#### `~/.config/telos/config.yaml` の主要項目
...
| `TELOS_HOME` | データディレクトリパス（デフォルト: `<telos.yaml のディレクトリ>/data/`） |
| `TELOS_GLOBAL_CONFIG` | グローバル設定ファイルのパス（デフォルト: `~/.config/telos/config.yaml`） |
...
| コマンド | 機能 |
|---|---|
| `telos init [--force] [--non-interactive]` | プロジェクト初期化ウィザード（APIキー検証・`telos.yaml` 書き込み・Docker 起動・埋め込みモデル DL）。`--force` で強制上書き、`--non-interactive` で全プロンプトスキップ（CI用） |
| `telos init --global [--force]` | グローバル設定初期化（`~/.config/telos/config.yaml` を生成）。マシンごとに1回実行 |
...
各プロジェクトは独立した DB・ワークスペース・ログを持つ。`TELOS_HOME` でデータディレクトリを指定する。デフォルトは `telos.yaml` があるディレクトリの `data/`（既存の `projects/main/` が存在する場合はそちらを優先）。

`telos project switch` は `TELOS_HOME=<path>` を `.env.local` に書き込み、次回起動から有効になる。

```
my-project/
├── telos.yaml          ← プロジェクト設定（git管理対象）
├── rubric.json
├── data/               ← TELOS_HOME のデフォルト（git管理外）
│   ├── telos.db
│   ├── JOURNAL.md
│   ├── workspace/
│   └── agent.log
...
[Loop 1] ✅ 0.81 — ファイルシステムの構造を調査する
[Loop 2] Generating goal...
[Loop 2] Goal: 外部APIへの接続を試みる
[Loop 2] ❌ 0.21 — 外部APIへの接続を試みる
[Session] Complete: 2 loops | avg: 0.51 | $0.031 | JOURNAL updated
```
...
| ファイル | テスト対象 |
|---|---|
| `test_config.py` | 設定システム: `_deep_merge`, `_find_project_config`（上方向探索・後方互換）, `load_settings`（グローバルのみ・プロジェクトのみ・マージ・env var優先） |
...
| 原因 | 確認方法 | 対処 |
|---|---|---|
| LLMが計画テキストを返してループが終了した（旧バグ、修正済み） | `JOURNAL.md` の Result/Reasoning を確認 | `had_tool_call` フラグで修正済み |
| Producerがファイルを書き込まなかった | `JOURNAL.md` の Result が空でないか確認 | `producer_system.txt` の指示を確認 |
