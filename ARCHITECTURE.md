...
- **方針**: モデル非依存（litellm経由でOpenAI/Anthropic/Gemini/OpenRouter/Ollama等すべて対応）
...
| 埋め込みモデルの次元数 | `config.yaml` (`embedding_dimensions`) または `_KNOWN_DIMENSIONS` 辞書 |
...
LLMによって返却形式が異なるため、`chat_structured` 内で以下の順序で正規化する：

```
生のJSONデータ (data)
  │
  ├─ 1. "arguments" ラッパー除去
  │      data["arguments"] が dict なら data = data["arguments"]
  │
  ├─ 2. "scores" / "evaluation" フラット化
  │      data["scores"] が dict なら data.update(data.pop("scores"))
  │      data["evaluation"] が dict なら data.update(data.pop("evaluation"))
  │
  ├─ 3. フラット数値フィールド → scores 再ネスト（EvaluationResponse用）
  │      response_model に "scores" フィールドがあるが data に "scores" がない場合:
  │      既知のフィールド以外の数値をすべて収集して data["scores"] に格納
  │      （DeepSeekなどが scores を展開して返す挙動への対応）
  │
  └─ 4. response_model(**data) でPydanticバリデーション
```
...
### 4.9 設定システム (`config.py`, `config.yaml`)
...
```yaml
llm:
  producer_model: openrouter/deepseek/deepseek-chat
  goal_gen_model: openrouter/deepseek/deepseek-chat
  critic_model: openrouter/deepseek/deepseek-chat
  max_tokens_per_loop: 50000        # 累積トークン上限（クラウドモデルは50000推奨）
...
max_output_truncation: 8000         # ツール出力の最大文字数
...
- **パストラバーサル保護**: `_resolve_safe_path()` で全パスをワークスペース内に制限（絶対パスは先頭 `/` を除去して相対化）
- **コスト上限**: 日次ループ数 + 月次コストの二重ガード（超過は `RuntimeError` で即停止）
- **コマンドインジェクション防止**: `LocalSandboxStrategy` は `shlex.split` + `shell=False`
- **自己コード変更不可**: `ToolRegistry` は静的。エージェントはTelos自身のコードを変更できない
- **致命的クォータエラー即停止**: `spending cap` / `budget exceeded` はリトライせず即時終了
