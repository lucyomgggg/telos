...
├── projects/                   # 各プロジェクトのデータディレクトリ
│   └── default/                # デフォルトプロジェクト（TELOS_HOME のデフォルト、gitignore）
...
# > 0.7: "high (starving for novelty)", < 0.3: "low (satisfied)", else: "moderate"
instinct_section = """
Current Instinct State:
- Curiosity:     0.82 — starving for novelty
- Preservation:  0.15 — system healthy
- Growth:        0.64 — moderate
- Order:         0.41 — moderate
"""
```
...
`workspace/persistent/` は半永続的に維持される。エージェントは前のループで作成したコードやライブラリを次のループでも継続して利用・改良できる。
...
```
直近10ループの失敗率（`exit_code != 0` または `status == "failed"`）とタイムアウト率から算出。実行が不安定なほど `preservation` が高くなり、Goal Gen に慎重な目標を生成させる。

**Growth** — 成長欲求（高 = 停滞 → 複雑化を求める）
```
complexity = LOC + 5 * function_count + 3 * import_count
growth = 1.0 - sigmoid((newer_avg - older_avg) / max(older_avg, 1.0) * 3)
```
...
order = min(isolation_ratio * 1.5, 1.0)
```
...
| (avg_score) | float | (廃止) クリティカルな評価の平均スコア |
| (critic_model) | str | (廃止) 使用したCriticモデル名 |
...
| (score) | float | (廃止) 以前の評価スコア |
| (reasoning) | str | (廃止) 目標達成の推論や評価理由 |
...
| `TELOS_HOME` | データディレクトリパス（デフォルト: `./projects/default`） |
...
`telos project switch` は `TELOS_HOME=projects/<name>` を `.env.local` に書き込み、次回起動から有効になる。`TELOS_HOME` 未設定時のデフォルトは `./projects/default`。

```
telos/                      ← リポジトリルート
├── config.yaml             ← インフラ設定（git管理）
├── telos.yaml              ← プロジェクト設定（git管理）
└── projects/               ← TELOS_HOME（gitignore）
    ├── default/            ← TELOS_HOME デフォルト
    │   ├── telos.db
    │   ├── JOURNAL.md
    │   ├── workspace/
    │   └── agent.log
